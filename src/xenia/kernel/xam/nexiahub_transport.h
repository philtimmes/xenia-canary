...
class NexiaHubTransport {
 public:
  ...
  bool Rendezvous(const std::string& room_key, std::string& other_host,
                  uint16_t& other_port);
  ...
  bool ConnectTcpTunnel(const std::string& relay_host, uint16_t relay_port,
                        const std::string& room_id, const char* role,
  ...
  bool ConnectTcpTunnel(const std::string& relay_host, uint16_t relay_port,
                        const char* role, uint16_t udp_port_hint);
  ...
  int TunnelRecv(std::string& host_out, uint16_t& port_out, void* buf,
                 size_t buf_cap, size_t& nread, bool nonblock);
 ...
 private:
  ...
  static bool TcpPost(const char* host, uint16_t port, const char* path,
                      const std::string& body, std::string& resp_out);
